...
const GEITA_BRANCH_LOCATION = BRANCH_GEITA;

const isGeitaBranchPart = (part) =>
  String(part?.location || '').trim().toLowerCase() === GEITA_BRANCH_LOCATION.toLowerCase();
...
function ManagerSpareparts() {
  ...
  const geitaSpareParts = spareParts.filter(isGeitaBranchPart);
  ...
  const totalParts = geitaSpareParts.length;
  const lowStockCount = geitaSpareParts.filter((p) => (Number(p.quantity) || 0) < LOW_STOCK_THRESHOLD).length;
  ...
  const buildSparepartsReportHtml = () => {
    ...
    const printedBy = safe(geitaUserName(user));
        ...
        <body>
          <div class="top">
            <div class="left">
              ...
              ${getPrintCompanyHtml("company", BRAND_ADDRESS_GEITA)}
            </div>
            <div class="meta">
              <p><strong>Report:</strong> ${safe(geitaLabels.sparePartsInventoryReport)}</p>
  ...
  const handleDownloadSpareparts = () => {
    ...
    downloadHtmlDocument(html, `geita-spare-parts-inventory-${new Date().toISOString().slice(0, 10)}.html`);
  };
  ...
  const handleAddSpare = async (e) => {
    ...
    if (retailPrice == null || Number.isNaN(retailPrice) || retailPrice < 0) {
      Swal.fire({ icon: 'warning', title: 'Invalid', text: 'Retail price must be a valid number (≥ 0).', confirmButtonColor: colors.primary });
      return;
    }
  ...
  return (
    <div className="payments-container geita-portal">
      ...
      <div className="main-content">
        <GeitaPageHeader
          title={geitaLabels.pageTitles.spareParts}
          user={user}
