...
function ManagerTransactions() {
  ...
  useEffect(() => {
    ...
    const loadSpareparts = async () => {
      try {
        const response = await getSpareParts(BRANCH_BOMA);
...
<body>
  ...
  const handleSaveReceiveMoney = async () => {
    ...
    try {
      ...
      const refreshResponse = await getPayments({ location: BRANCH_BOMA });
  ...
  const handleSaveEditableItems = async () => {
    ...
    try {
      ...
      const paymentsResponse = await getPayments({ location: BRANCH_BOMA });
  ...
  return (
    <div className="payments-container boma-portal">
      ...
      <div className="main-content">
        <BomaPageHeader
          title={bomaLabels.pageTitles.transactions}
          user={user}
